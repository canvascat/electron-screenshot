...
export function hsvFormatHex(
  ...
  const a =
    typeof alpha === 'number'
      ? (((alpha * 255) / 100) | 0).toString(16).padStart(2, '0')
      : '';
  return `#${h}${e}${x}${a}`;
}
...
const rgb2hex = (r: number, g: number, b: number) =>
  <HexArray>[r, g, b].map((v) => v.toString(16).padStart(2, '0'));
...
export function hsv2hex(h: number, s: number, v: number) {
  return hsv2rgb(h, s, v).map((value) =>
    Math.min(value | 0, 255)
      .toString(16)
      .padStart(2, '0')
  );
}
  ...
  fromString(val: string) {
    ...
    if (val.indexOf('hsl') !== -1) {
      ...
      const parts = val
        ...
        .split(/\s|,/g)
        .filter((v) => v !== '')
        .map((v, index) => (index > 2 ? parseFloat(v) : parseInt(v, 10)));
      ...
      const [r, g, b, a] = range(len / channelLen).map((i) =>
        parseHexChannel(hex.substr(i * channelLen, channelLen))
      );
    ...
    if (this.enableAlpha) {
      switch (format) {
        case 'hsl': {
          ...
          this.color = `hsla(${hue}, ${hsl[1] | 0}%, ${hsl[2] | 0}%, ${
            alpha / 100
          })`;
          break;
        }
        case 'hsv': {
          this.color = `hsva(${hue}, ${saturation | 0}%, ${value | 0}%, ${
            alpha / 100
          })`;
          break;
        }
      ...
      switch (format) {
        ...
        default: {
          this.color =
            '#' + rgb2hex(...hsv2rgb(hue, saturation, value)).join('');
        }
      }
    }
